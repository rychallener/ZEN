...
"""
    ...
    2012-03-17  patricio  updated for preflash, postcal handling
"""


def saveevent(event, filename, save=[], delete=[]):
  """
    Saves an event in .dat (using cpickle) and .h5 (using h5py) files.
    ...
    Side efect: This routine deletes all parameters except 'event' 
                after saving it.
  ...
  handle = open(filename + '.dat', 'w')
  pickle.dump(event, handle)
  handle.close()


def loadevent(filename, load=[]):
  """
    Loads an event stored in .dat and .h5 files.
  ...
  handle = open(filename + '.dat', 'r')
  event = pickle.load(handle)
  handle.close()

  if load != []:
    handle = h5.File(filename + '.h5', 'r')
    for param in load:
